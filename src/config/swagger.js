const swaggerDefinition = {
    ...
    paths: {
        ...
        , '/v1/appointments': {
            ...
            post: { tags: ['Appointments'], summary: 'Create appointment booking', description: 'Books the selected available doctor slot returned by GET /v1/doctors/{doctorId}/slots.', security: [{ bearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['doctorId', 'date', 'slotId', 'consultationType'], properties: { doctorId: { type: 'string', example: 'doc_101' }, date: { type: 'string', format: 'date', example: '2026-09-24' }, slotId: { type: 'string', example: 'doc_101_2026-09-24_0900' }, consultationType: { type: 'string', enum: ['VIDEO', 'AUDIO', 'CHAT'], example: 'VIDEO' }, symptoms: { type: 'string', nullable: true } }, example: { doctorId: 'doc_101', date: '2026-09-24', slotId: 'doc_101_2026-09-24_0900', consultationType: 'VIDEO', symptoms: 'Headache' } } } } }, responses: { 201: { description: 'Appointment created with pending payment', content: { 'application/json': { schema: { type: 'object', required: ['success', 'data'], properties: { success: { type: 'boolean', example: true }, message: { type: 'string' }, data: { $ref: '#/components/schemas/Appointment' } } } } } }, 400: { $ref: '#/components/responses/BadRequest' }, 401: { $ref: '#/components/responses/Unauthorized' }, 403: { $ref: '#/components/responses/Forbidden' }, 404: { $ref: '#/components/responses/NotFound' }, 409: { $ref: '#/components/responses/Conflict' } } }
        },
