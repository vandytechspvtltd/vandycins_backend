const swaggerDefinition = {
    ...
    components: {
        ...
        schemas: {
            ...
            Profile: {
                type: 'object', required: ['name', 'dob', 'blood_group', 'gender', 'email', 'city'],
                properties: {
                    name: { type: 'string' }, dob: { type: 'string', format: 'date' }, blood_group: { type: 'string', enum: ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'] }, gender: { type: 'string', enum: ['MALE', 'FEMALE', 'OTHER'] }, email: { type: 'string', format: 'email' }, city: { type: 'string' }, avatar: { type: 'string', nullable: true }, latitude: { type: 'number', minimum: -90, maximum: 90, nullable: true }, longitude: { type: 'number', minimum: -180, maximum: 180, nullable: true }, age: { type: 'integer' }
                }
            },
            Doctor: {
                type: 'object', properties: {
            ...
            AppointmentSlot: {
                type: 'object', properties: {
                    id: { type: 'string' }, time: { type: 'string' }, period: { type: 'string', enum: ['MORNING', 'AFTERNOON', 'EVENING'] }, isAvailable: { type: 'boolean' }
                }
            }
        }
    },
    paths: {
        ...
        '/v1/doctors/{doctorId}/slots': { get: { tags: ['Doctors'], summary: 'Get available doctor slots', security: [{ bearerAuth: [] }], parameters: [{ name: 'doctorId', in: 'path', required: true, schema: { type: 'string' } }, { name: 'date', in: 'query', required: false, schema: { type: 'string', format: 'date' } }], responses: { 200: { description: 'Available slots' }, 400: { $ref: '#/components/responses/BadRequest' }, 401: { $ref: '#/components/responses/Unauthorized' }, 404: { $ref: '#/components/responses/NotFound' } } } },
        ...
        , '/v1/appointments': {
            get: { tags: ['Appointments'], summary: 'List patient appointments', security: [{ bearerAuth: [] }], responses: { 200: { description: 'Patient appointments' }, 401: { $ref: '#/components/responses/Unauthorized' } } },
            post: { tags: ['Appointments'], summary: 'Book appointment', security: [{ bearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['doctorId', 'date', 'time'], properties: { doctorId: { type: 'string' }, date: { type: 'string', format: 'date' }, time: { type: 'string' }, type: { type: 'string', enum: ['VIDEO', 'AUDIO', 'CHAT'], default: 'VIDEO' }, symptoms: { type: 'string', nullable: true } } } } } }, responses: { 201: { description: 'Appointment created' }, 400: { $ref: '#/components/responses/BadRequest' }, 401: { $ref: '#/components/responses/Unauthorized' }, 404: { $ref: '#/components/responses/NotFound' }, 409: { $ref: '#/components/responses/Conflict' } } }
        },
        '/v1/appointments/{appointmentId}': { get: { tags: ['Appointments'], summary: 'Get patient appointment', security: [{ bearerAuth: [] }], parameters: [{ name: 'appointmentId', in: 'path', required: true, schema: { type: 'string' } }], responses: { 200: { description: 'Appointment details' }, 401: { $ref: '#/components/responses/Unauthorized' }, 404: { $ref: '#/components/responses/NotFound' } } } },
        '/v1/appointments/{appointmentId}/cancel': { put: { tags: ['Appointments'], summary: 'Cancel patient appointment', security: [{ bearerAuth: [] }], parameters: [{ name: 'appointmentId', in: 'path', required: true, schema: { type: 'string' } }], responses: { 200: { description: 'Appointment cancelled' }, 401: { $ref: '#/components/responses/Unauthorized' }, 404: { $ref: '#/components/responses/NotFound' }, 409: { $ref: '#/components/responses/Conflict' } } } },
